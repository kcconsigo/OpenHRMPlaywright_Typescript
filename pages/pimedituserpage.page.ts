...
export class PIMEditUserPage {
    ...
    async employeeListlandingTab(editusername: string) {
        await this.page.locator(pimlocators.listEmployeeName).nth(0).getByPlaceholder('Type for hints...')
            .nth(0)
            .fill(editusername);
        const Empnames: Locator = this.page.locator(pimlocators.listEmployeeNameSelect).getByRole('option', { name: editusername });
